...
void QRCodeDialog::setModel(OptionsModel *model) {
    ...
    // update the display unit, to not use the default ("ATH")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI() {
    ...
    if (ui->chkReqPayment->isChecked()) {
        if (ui->lnReqAmount->validate()) {
            // even if we allow a non ATH unit input in lnReqAmount, we generate the URI with ATH as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ATH, ui->lnReqAmount->value()));
            paramCount++;
        } else {
            ui->btnSaveAs->setEnabled(false);
