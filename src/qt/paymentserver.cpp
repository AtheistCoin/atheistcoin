...
const QString BITCOIN_IPC_PREFIX("AtheistCoin:");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true) {
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click AtheistCoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start AtheistCoin: click-to-pay handler");
    else
        connect(uriServer, SIGNAL(newConnection()), this, SLOT(handleURIConnection()));
}

bool PaymentServer::eventFilter(QObject *object, QEvent *event) {
    // clicking on AtheistCoin: URLs creates FileOpen events on the Mac:
