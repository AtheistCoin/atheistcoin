...
static inline unsigned short GetDefaultRPCPort() {
    return GetBoolArg("-testnet", false) ? 33841 : 33941;
}
...
Value stop(const Array& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop AtheistCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "AtheistCoin server stopping";
}
...
static const CRPCCommand vRPCCommands[] = {//  name                      actor (function)         okSafeMode threadSafe
    //  ------------------------  -----------------------  ---------- ----------
    { "help", &help, true, true},
    { "stop", &stop, true, true},
...
string HTTPPost(const string& strMsg, const map<string, string>& mapRequestHeaders) {
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
            << "User-Agent: AtheistCoin-json-rpc/" << FormatFullVersion() << "\r\n"
            << "Host: 127.0.0.1\r\n"
...
void StartRPCThreads() {
    ...
    if ((mapArgs["-rpcpassword"] == "") ||
        ...
        string strWhatAmI = "To use AtheistCoind";
