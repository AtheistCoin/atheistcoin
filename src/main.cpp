...
// Copyright (c) 2014 AtheistCoin developers http://AtheistCoin.org
...
//---genesis blocks -------------------------------------------------------------------------------------
uint256 hashGenesisBlock("0x000006e6de1b18ceb161b92a3e493098320a72759ea52001e37459ea2738d066");
uint256 hashGenesisBlockTestNet("0x000005780ee7f9e7a5aa130fd4d75150eebfbb811410806a9fe6ac55220d5c34");
//-------------------------------------------------------------------------------------------------------
uint256 MainHashMerkleRoot("0xb75fb08db49ace708d1bd4d5772491e1266ed3118604c8bca83b608267cae6a1");
//--time blocks ---------------------------------------
static const unsigned int timeGenesisBlock = 1402331755;
static const unsigned int timeGenesisBlockTestNet = 1400020893;
//-------nNonce -------------------------------------
static const unsigned int nNonceGenesis = 1548132;
static const unsigned int nNonceGenesisTestNet = 1782173;
//----------------------------------------------------
...
const string strMessageMagic = "AtheistCoin Signed Message:\n";
...
bool CCoinsView::BatchWrite(const std::map<uint256, CCoins> &mapCoins, CBlockIndex *pindex) {
    return false;
}
...
bool CCoinsViewBacked::BatchWrite(const std::map<uint256, CCoins> &mapCoins, CBlockIndex *pindex) {
...
static const int64 nTargetSpacing = 45; // atheistcoin 45 second blocks
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The birth of AtheistCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        if (fTestNet) {
            ...
            block.nNonce = nNonceGenesisTestNet;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == MainHashMerkleRoot);
        ...
        BOOST_FOREACH(CAddress& addr, vAddr) {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable()) {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = HashGroestl(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes) {
                        ...
                        hashKey = HashGroestl(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom) {
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = HashGroestl(vRecv.begin(), vRecv.begin() + nMessageSize);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AtheistCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AtheistCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet) {
    printf("AtheistCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("AtheistCoin-miner");
    ...
    try {
        while (true) {
            ...
            printf("Running AtheistCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        printf("AtheistCoinMiner terminated\n");
        throw;
    }
}
